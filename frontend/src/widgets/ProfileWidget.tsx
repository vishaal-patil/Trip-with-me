    ...
    return (
        <div className="w-full h-full p-4 md:p-6 pb-24 md:pb-6 overflow-y-auto">
            <div className="flex flex-col gap-6 max-w-7xl mx-auto">
                ...
                <GlassCard className="relative overflow-hidden p-0 group shrink-0">
                    ...
                    <div className="px-6 pb-6 -mt-12 flex flex-col items-center md:items-start md:flex-row gap-4 relative z-10">
                        <Avatar src={user.avatar} size="xl" bordered className="border-4 border-background w-32 h-32 md:w-32 md:h-32" />
                        ...
                        <div className="pt-2 md:pt-14 flex flex-col items-center md:items-end gap-2 w-full md:w-auto">
                            <div className="flex items-center gap-2">
                                <Badge label={user.level} variant="gold" icon={<Award className="w-3 h-3" />} />
                                <span className="text-xs text-secondary">{user.xp} XP</span>
                            </div>
                            <div className="w-full md:w-48 h-2 bg-white/10 rounded-full overflow-hidden">
                                <div className="h-full bg-accent" style={{ width: `${progress}%` }} />
                            </div>
                            <div className="text-xs text-secondary text-right w-full">Next: {user.nextLevelXp} XP</div>
                        </div>
                    </div>
                </GlassCard>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                    ...
                    {/* Right Column: Interactive Content (Map Placeholder) */}
                    <div className="flex flex-col gap-6 h-full">
                        <GlassCard className="p-1 flex-1 min-h-[300px] overflow-hidden relative group h-full">
                            <img
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-black/20 group-hover:bg-black/40 transition-colors">
                                <h3 className="text-xl font-bold text-white drop-shadow-lg">My World Map</h3>
                                <GradientButton>Explore Interactive Map</GradientButton>
                            </div>
                        </GlassCard>
                    </div>
                </div>
